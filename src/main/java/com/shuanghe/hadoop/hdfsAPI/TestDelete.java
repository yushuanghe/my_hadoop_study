...
//                  delete();
     ...
     * @throws IOException
     */
    private static void deleteOnExit() throws IOException {
        FileSystem fs = HdfsUtil.getFileSystem();
        boolean deleted = fs.delete(new Path("api/copyFromLocal.xml"), true);
        System.out.println("delete:" + (deleted ? "删除成功" : "删除失败"));

        deleted = fs.deleteOnExit(new Path("api/create2.txt"));
