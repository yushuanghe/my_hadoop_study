...
public class AreaTop3ProductSpark {
        ...
        JavaRDD<Row> mappedRDD = joinedRDD.map(
                new Function<Tuple2<Long, Tuple2<Row, Row>>, Row>() {
                    ...
                    public Row call(Tuple2<Long, Tuple2<Row, Row>> tuple)
                        ...
                        long productid = clickAction.getAs("product_id");
                        String cityName = cityInfo.getAs("city_name");
                        String area = cityInfo.getAs("area");
    ...
    private static void generateTempAreaPrdocutClickCountTable(SQLContext sqlContext) {
        ...
        String sql =
                "SELECT "
                        ...
                        + "group_concat_distinct(concat_long_string(city_id,city_name,':')) AS city_infos "
    ...
    private static void persistAreaTop3Product(long taskid, List<Row> rows) {
        ...
        for (Row row : rows) {
            ...
            areaTop3Product.setArea(row.getAs("area"));
            areaTop3Product.setAreaLevel(row.getAs("area_level"));
            areaTop3Product.setProductid(row.getAs("product_id"));
            areaTop3Product.setClickCount(row.getAs("click_count"));
            areaTop3Product.setCityInfos(row.getAs("city_infos"));
            areaTop3Product.setProductName(row.getAs("product_name"));
            areaTop3Product.setProductStatus(row.getAs("product_status"));
