 ...
 * <p>
 * 我们的spark作业如何接收用户创建的任务？
 * <p>
 * J2EE平台在接收用户创建任务的请求之后，会将任务信息插入MySQL的task表中，任务参数以JSON格式封装在 task_param 字段中
 * <p>
...
public class UserVisitSessionAnalyzeSpark {
    ...
    public static void main(String[] args) {
        ...
        //sc.checkpointFile("hdfs://shuanghe.com:8020/user/yushuanghe/spark-project");
        ...
        /*
         如果要进行session粒度的数据聚合
         首先要从 user_visit_action 表中，查询出来指定日期范围内的行为数据
        */
        ...
        */
        ...
        /*
         ...
         此时的数据的粒度就是session粒度了，然后呢，可以将session粒度的数据与用户信息数据，进行join
         然后就可以获取到session粒度的数据，同时呢，数据里面还包含了session对应的user的信息
         到这里为止，获取的数据是<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,gender)>
        */
        ...
        */
        ...
        // 生成公共的RDD：通过筛选条件的session对应的访问明细数据,使用join进行过滤
        // <sessionid,row>
        ...
        /*
         ...
         是不是在 calculateAndPersisitAggrStat 方法之后，运行一个action操作，比如count、take
         不对！！！
    ...
    public static JavaPairRDD<String, Row> getSessionid2ActionRDD(JavaRDD<Row> actionRDD) {
        ...
        //        return new Tuple2<>(row.getAs("session_id"), row);
        //    }
        //
        //});

        //使用 mapPartitions 算子提升性能
        return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {
            ...
            public Iterator<Tuple2<String, Row>> call(Iterator<Row> rowIterator) throws Exception {
                ...
                while (rowIterator.hasNext()) {
                    ...
                    list.add(new Tuple2<>(row.getAs("session_id"), row));
                }
    ...
    private static JavaPairRDD<String, String> aggregateBySession(
                ...
                new PairFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
                    ...
                    public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (userid == null) {
                                userid = row.getAs("user_id");
                            }
                            String searchKeyword = row.getAs("search_keyword");
                            ...
                            */
                            ...
                            // 计算session开始和结束时间
                            Date actionTime = DateUtils.parseTime(row.getAs("action_time"));
        ...
        JavaPairRDD<Long, Row> userid2InfoRDD = userInfoRDD.mapToPair(
                ...
                new PairFunction<Row, Long, Row>() {
                    ...
                    public Tuple2<Long, Row> call(Row row) throws Exception {
                        return new Tuple2<>(row.getAs("user_id"), row);
                    }
        ...
        JavaPairRDD<String, String> sessionid2FullAggrInfoRDD = userid2FullInfoRDD.mapToPair(
                ...
                new PairFunction<Tuple2<Long, Tuple2<String, Row>>, String, String>() {
                    ...
                    public Tuple2<String, String> call(
                        ...
                        int age = userInfoRow.getAs("age");
                        String professional = userInfoRow.getAs("professional");
                        String city = userInfoRow.getAs("city");
                        String gender = userInfoRow.getAs("gender");
        ...
        //
        ////倾斜RDD join
        ////将倾斜key的 userid2InfoRDD 扩容100倍,添加随机数前缀,减轻数据倾斜
        ...
        //
        //    @Override
        //    public Iterator<Tuple2<String, Row>> call(
         ...
         * 使用随机数和扩容表进行join
         */
        //JavaPairRDD<String, Row> expandedRDD = userid2InfoRDD
        //        //选择其中较小的rdd扩容10倍
        //        .flatMapToPair(
        //
        ...
        //
        //                    @Override
        //                    public Iterator<Tuple2<String, Row>> call(Tuple2<Long, Row> tuple)
        //                            throws Exception {
        //                        List<Tuple2<String, Row>> list = new ArrayList<>();
        //                        Random random = new Random();
        //                        for (int i = 0; i < 10; i++) {
    ...
    private static JavaPairRDD<String, String> filterSessionAndAggrStat(
        ...
        */
        ...
        String sex = ParamUtils.getParam(taskParam, Constants.PARAM_GENDER);
        ...
        String _parameter = (startAge != null ? Constants.PARAM_START_AGE + "=" + startAge + "|" : "")
                ...
                + (sex != null ? Constants.PARAM_GENDER + "=" + sex + "|" : "")
                ...
                new Function<Tuple2<String, String>, Boolean>() {
                    ...
                    public Boolean call(Tuple2<String, String> tuple) throws Exception {
                        ...
                        if (!ValidUtils.equal(aggrInfo, Constants.FIELD_GENDER,
                                parameter, Constants.FIELD_GENDER)) {
                            return false;
                        }
     ...
     * <p>
     * 1、session聚合数据进行映射，将每个session发生的yyyy-MM-dd_HH（start_time）作为key，value就是session_id
     * 对上述数据，使用countByKey算子，就可以获取到每天每小时的session数量
     * <p>
     * 2、（按时间比例随机抽取算法）每天每小时有多少session，根据这个数量计算出每天每小时的session占比，以及按照占比，需要抽取多少session，可以计算出每个小时内，从0~session数量之间的范围中，获取指定抽取数量个随机数，作为随机抽取的索引
     * <p>
    ...
    private static void randomExtractSession(
        ...
        /*
         ...
         首先抽取出的session的聚合数据，写入 session_random_extract 表
        ...
        /*
         ...
         那么抽取该session，直接写入MySQL的 session_random_extract 表
    ...
    private static List<Tuple2<CategorySortKey, String>> getTop10Category(
        ...
        JavaPairRDD<Long, Long> categoryidRDD = sessionid2detailRDD.flatMapToPair(
                ...
                new PairFlatMapFunction<Tuple2<String, Row>, Long, Long>() {
                    ...
                    public Iterator<Tuple2<Long, Long>> call(
                        ...
                        Set<Tuple2<Long, Long>> set = new HashSet<>();

                        Long clickCategoryId = row.getAs("click_category_id");
                        if (clickCategoryId != null) {
                            set.add(new Tuple2<>(clickCategoryId, clickCategoryId));
                        }

                        String orderCategoryIds = row.getAs("order_category_ids");
                        if (orderCategoryIds != null) {
                            String[] orderCategoryIdsSplited = orderCategoryIds.split(",");
                            for (String orderCategoryId : orderCategoryIdsSplited) {
                                set.add(new Tuple2<>(Long.valueOf(orderCategoryId),
                                        Long.valueOf(orderCategoryId)));
                            }
                        }

                        String payCategoryIds = row.getAs("pay_category_ids");
                        if (payCategoryIds != null) {
                            String[] payCategoryIdsSplited = payCategoryIds.split(",");
                            for (String payCategoryId : payCategoryIdsSplited) {
                                set.add(new Tuple2<>(Long.valueOf(payCategoryId),
        ...
        top10CategoryList.forEach((x) -> System.out.println(x));
        ...
        JavaPairRDD<String, Row> clickActionRDD = sessionid2detailRDD.filter(
                ...
                new Function<Tuple2<String, Row>, Boolean>() {
                    ...
                    public Boolean call(Tuple2<String, Row> tuple) throws Exception {
                        Row row = tuple._2;
                        return row.getAs("click_category_id") != null ? true : false;
                    }

                })
                //filter之后使用coalesce平衡各partition数据,避免数据倾斜
                //.coalesce(100)
                ;
        ...
        JavaPairRDD<Long, Long> clickCategoryIdRDD = clickActionRDD.mapToPair(
                ...
                new PairFunction<Tuple2<String, Row>, Long, Long>() {
                    ...
                    public Tuple2<Long, Long> call(Tuple2<String, Row> tuple)
                            throws Exception {
                        long clickCategoryId = tuple._2.getAs("click_category_id");
    ...
    private static JavaPairRDD<Long, Long> getOrderCategoryId2CountRDD(
        ...
        JavaPairRDD<String, Row> orderActionRDD = sessionid2detailRDD.filter(
                ...
                new Function<Tuple2<String, Row>, Boolean>() {
                    ...
                    public Boolean call(Tuple2<String, Row> tuple) throws Exception {
                        Row row = tuple._2;
                        return row.getAs("order_category_ids") != null ? true : false;
                    }
    ...
    private static JavaPairRDD<Long, Long> getPayCategoryId2CountRDD(
        ...
        JavaPairRDD<String, Row> payActionRDD = sessionid2detailRDD.filter(
                ...
                new Function<Tuple2<String, Row>, Boolean>() {
                    ...
                    public Boolean call(Tuple2<String, Row> tuple) throws Exception {
                        Row row = tuple._2;
                        return row.getAs("pay_category_ids") != null ? true : false;
                    }
    ...
    private static JavaPairRDD<Long, String> joinCategoryAndData(
        ...
        /*
         解释一下，如果用 leftOuterJoin ，就可能出现，右边那个RDD中，join过来时，没有值
                ...
                new PairFlatMapFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
                    ...
                    public Iterator<Tuple2<Long, String>> call(
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (row.getAs("click_category_id") != null) {
                                long categoryid = row.getAs("click_category_id");
